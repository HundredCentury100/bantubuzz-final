...
const CampaignPaymentModal = ({ isOpen, onClose, campaign, selectedCollaborations, onPaymentSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && selectedCollaborations.length > 0) {
      calculatePayment();
    }
  }, [isOpen, selectedCollaborations]);

  const calculatePayment = async () => {
    try {
      setCalculating(true);
      const response = await campaignPaymentsAPI.calculate({
        campaign_id: campaign.id,
        collaboration_ids: selectedCollaborations.map(c => c.id),
        payment_type: selectedCollaborations.length === 1 ? 'individual' : 'batch'
      });
  ...
  const handlePayment = async () => {
    try {
      ...
      const response = await campaignPaymentsAPI.initiate({
        campaign_id: campaign.id,
        collaboration_ids: selectedCollaborations.map(c => c.id),
        payment_type: selectedCollaborations.length === 1 ? 'individual' : 'batch',
  ...
  const handleProofUpload = async () => {
    ...
    try {
      ...
      // await campaignPaymentsAPI.uploadProof(payment_id, formData);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Pay Creators</h2>
          ...
          {calculating ? (
            ...
            <div className="space-y-2">
                ...
                {calculation.items.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-sm bg-white p-2 rounded">
                    <span className="text-gray-700">
                      {item.creator_name} - {item.package_title}
                    </span>
              ...
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Subtotal ({calculation.items_count} creator{calculation.items_count > 1 ? 's' : ''})</span>
                <span className="font-medium">${calculation.subtotal}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Platform Fee (10%)</span>
                <span className="font-medium">${calculation.platform_fee.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-lg font-bold pt-2 border-t border-gray-200">
                <span>Total Amount</span>
                <span className="text-primary">${calculation.total_amount.toFixed(2)}</span>
              </div>
            </div>
          ) : null}
        </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {processing ? (
                    ...
                    <>
                      Pay ${calculation?.total_amount.toFixed(2) || '0.00'}
                    </>
                  )}
                </button>
              </div>
      ...
      <SmilePayPaymentModal
        ...
        currency="USD"
        paymentType="campaign_payment"
        paymentId={campaign?.id}
        itemName={`Campaign: ${campaign?.title}`}
        itemDescription={`Payment for ${selectedCollaborations.length} creator${selectedCollaborations.length > 1 ? 's' : ''}`}
        onSuccess={handleSmilePaySuccess}
        returnUrl={`${window.location.origin}/brand/campaigns/${campaign?.id}`}
