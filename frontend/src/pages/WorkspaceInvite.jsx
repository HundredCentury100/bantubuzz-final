...
const WorkspaceInvite = () => {
  ...
  useEffect(() => {
    const fetchInvitation = async () => {
      try {
        setLoading(true);
        const response = await workspacesAPI.getInvitation(token);
  ...
  const handleAccept = async () => {
    try {
      ...
      localStorage.setItem('selected_workspace_id', String(response.data.workspace.id));
  ...
  const language = inviteData?.language || {};
  const workspaceLabel = language.workspace_singular || 'workspace';
            ...
            {loading ? (
              ...
              <>
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/15">
                  <BuildingOffice2Icon className="h-7 w-7 text-dark" />
                </div>

                <h1 className="mt-5 text-3xl font-bold text-dark">You have been invited</h1>
                <p className="mt-3 text-gray-600">
                  Join <span className="font-semibold text-dark">{workspace?.name}</span> as a{' '}
                  <span className="font-semibold text-dark">{invitation?.role}</span> on this {workspaceLabel} workspace.
                </p>
                ...
                {!isAuthenticated && (
                  <div className="mt-6 rounded-2xl border border-amber-100 bg-amber-50 p-5">
                    ...
                    <p className="mt-1 text-sm text-gray-600">
                      Use the email address that received this invitation. New team members can create a brand account first.
                    </p>
                    <div className="mt-4 flex flex-wrap gap-3">
                      <Link to="/login" className="btn btn-primary">
                        Sign In
                      </Link>
                      <Link to="/register/brand" className="btn btn-secondary">
