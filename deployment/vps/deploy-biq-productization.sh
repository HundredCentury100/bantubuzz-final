...
BACKUP="/var/backups/bantubuzz/biq-productization-before-$TS"
...
tar -xzf /tmp/bantubuzz-biq-productization-backend.tar.gz -C backend
chown bantubuzz:www-data backend/app/services/creator_score_service.py
...
tar -xzf /tmp/bantubuzz-biq-productization-frontend.tar.gz -C "$FRONTEND_ROOT"
chown -R bantubuzz:www-data "$FRONTEND_ROOT"

echo "Compiling targeted backend file"
cd "$REMOTE_ROOT/backend"
venv/bin/python -m py_compile app/services/creator_score_service.py
...
rm -f \
  /tmp/bantubuzz-biq-productization-backend.tar.gz \
  /tmp/bantubuzz-biq-productization-frontend.tar.gz \
  /tmp/deploy-biq-productization.sh

echo BANTUBUZZ_NEW_VPS_BIQ_PRODUCTIZATION_SUCCESS
