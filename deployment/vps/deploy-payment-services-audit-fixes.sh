...
BACKUP="/var/backups/bantubuzz/payment-services-audit-fixes-before-$TS"
...
tar -xzf /tmp/bantubuzz-payment-services-audit-fixes-backend.tar.gz -C backend
chown -R bantubuzz:www-data \
  backend/app/models/campaign_payment.py \
  backend/app/routes/campaign_payments.py \
  backend/app/routes/creator_subscriptions.py \
...
tar -xzf /tmp/bantubuzz-payment-services-audit-fixes-frontend.tar.gz -C "$FRONTEND_ROOT"
...
rm -f \
  /tmp/bantubuzz-payment-services-audit-fixes-backend.tar.gz \
  /tmp/bantubuzz-payment-services-audit-fixes-frontend.tar.gz \
  /tmp/deploy-payment-services-audit-fixes.sh

echo BANTUBUZZ_NEW_VPS_PAYMENT_SERVICES_AUDIT_FIXES_SUCCESS
