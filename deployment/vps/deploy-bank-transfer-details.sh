...
BACKUP="/var/backups/bantubuzz/bank-transfer-details-before-$TS"
...
tar -xzf /tmp/bantubuzz-bank-transfer-details-backend.tar.gz -C backend
chown -R bantubuzz:www-data \
  backend/app/utils/bank_details.py \
...
tar -xzf /tmp/bantubuzz-bank-transfer-details-frontend.tar.gz -C "$FRONTEND_ROOT"
...
rm -f \
  /tmp/bantubuzz-bank-transfer-details-backend.tar.gz \
  /tmp/bantubuzz-bank-transfer-details-frontend.tar.gz \
  /tmp/deploy-bank-transfer-details.sh

echo BANTUBUZZ_NEW_VPS_BANK_TRANSFER_DETAILS_SUCCESS
