...
BACKUP="/var/backups/bantubuzz/creator-badges-before-$TS"
...
tar -xzf /tmp/bantubuzz-creator-badges-backend.tar.gz -C backend
rm -rf frontend/assets frontend/index.html frontend/favicon.ico frontend/manifest.json frontend/message-push-sw.js
tar -xzf /tmp/bantubuzz-creator-badges-frontend.tar.gz -C frontend
chown -R bantubuzz:www-data frontend backend/app/services backend/app/models backend/app/routes
...
rm -f \
  /tmp/bantubuzz-creator-badges-backend.tar.gz \
  /tmp/bantubuzz-creator-badges-frontend.tar.gz \
  /tmp/deploy-creator-badges.sh

echo BANTUBUZZ_NEW_VPS_CREATOR_BADGES_SUCCESS
