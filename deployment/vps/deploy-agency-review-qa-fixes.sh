...
FRONTEND_ARCHIVE="/tmp/bantubuzz-agency-review-qa-frontend.tar.gz"
BACKEND_ARCHIVE="/tmp/bantubuzz-agency-review-qa-backend.tar.gz"

echo "Creating targeted backup at ${BACKUP_ROOT}/agency-review-qa-before-${STAMP}"
mkdir -p "${BACKUP_ROOT}/agency-review-qa-before-${STAMP}"
if [ -d "$FRONTEND_DIR" ]; then
  tar -czf "${BACKUP_ROOT}/agency-review-qa-before-${STAMP}/frontend.tar.gz" -C "$FRONTEND_DIR" . 2>/dev/null || true
fi
tar -czf "${BACKUP_ROOT}/agency-review-qa-before-${STAMP}/backend-files.tar.gz" \
  -C "$BACKEND_DIR" \
...
chown -R www-data:www-data \
  "$BACKEND_DIR/app/routes/collaborations.py" \
  "$BACKEND_DIR/app/routes/workspaces.py" \
  || true

echo "Compiling backend files"
cd "$BACKEND_DIR"
venv/bin/python -m py_compile app/routes/collaborations.py app/routes/workspaces.py
...
echo "BANTUBUZZ_AGENCY_REVIEW_QA_FIXES_SUCCESS"
