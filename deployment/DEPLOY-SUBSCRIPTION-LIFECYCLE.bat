...
ssh %SERVER_USER%@%SERVER_HOST% "set -e; cd %REMOTE_ROOT%; rm -rf frontend/assets frontend/index.html frontend/favicon.ico frontend/manifest.json; tar -xzf /tmp/bantubuzz_frontend_subscription_lifecycle.tar.gz -C frontend; rm -f /tmp/bantubuzz_frontend_subscription_lifecycle.tar.gz; cd backend; source venv/bin/activate; python -m py_compile app/celery_app.py app/models/subscription.py app/models/subscription_plan.py app/routes/admin/payments.py app/routes/subscriptions.py app/services/email_service.py app/services/payment_service.py app/services/smilepay_service.py app/services/subscription_lifecycle_service.py app/tasks/subscription_tasks.py migrations/versions/202606051200_add_subscription_lifecycle_fields.py; flask db upgrade; pkill gunicorn || true; sleep 2; venv/bin/gunicorn -w 4 -b 0.0.0.0:8002 --timeout 120 --error-logfile gunicorn_error.log --access-logfile gunicorn_access.log 'app:create_app()' --daemon; (systemctl restart celery 2>/dev/null || true); (systemctl restart celerybeat 2>/dev/null || true); (pm2 restart celery 2>/dev/null || true); (pm2 restart celery-beat 2>/dev/null || true); systemctl restart apache2; sleep 3; echo 'Gunicorn processes:'; ps aux | grep '[g]unicorn'; echo 'Port 8002:'; (ss -tlnp | grep 8002 || netstat -tlnp | grep 8002)"
if errorlevel 1 goto fail
