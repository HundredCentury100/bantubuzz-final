...
echo ============================================
echo   BantuBuzz - Deploy via SCP (Direct Upload)
...
echo This script will upload your code directly via SCP
...
scp -r "%LOCAL_PATH%\backend" %SERVER_USER%@%SERVER_IP%:%DEPLOY_DIR%/
...
scp -r "%LOCAL_PATH%\frontend" %SERVER_USER%@%SERVER_IP%:%DEPLOY_DIR%/
...
echo.
echo To check status: ssh %SERVER_USER%@%SERVER_IP% "pm2 status"
echo To view logs: ssh %SERVER_USER%@%SERVER_IP% "pm2 logs"
echo.
