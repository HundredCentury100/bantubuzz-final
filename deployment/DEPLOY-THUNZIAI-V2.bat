...
scp "%ROOT%\deployment\restart-backend-remote.sh" %SERVER_USER%@%SERVER_HOST%:/tmp/bantubuzz_restart_backend.sh
if errorlevel 1 goto fail
ssh %SERVER_USER%@%SERVER_HOST% "chmod +x /tmp/bantubuzz_restart_backend.sh && REMOTE_BACKEND='%REMOTE_ROOT%/backend' PORT=8002 /tmp/bantubuzz_restart_backend.sh"
if errorlevel 1 goto fail
